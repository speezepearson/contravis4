...
export const pullByAnimator: InstructionAnimator<PullByInstruction> = (init, who, instr) => {
  ...
  return (t) => {
    ...
    return produce(init, (draft) => {
      ...
      for (const id of who) {
        const arc = plans[id];
        draft.protos[id].pos = ellipsePosition(arc.start, arc.end, semiMinorCw, Math.PI*progressFrac);
