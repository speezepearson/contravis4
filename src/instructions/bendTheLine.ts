...
> = (instr, init) => {
  ...
  for (const protoId of ALL_PROTO_IDS) {
    ...
    const refFacing = getDancer(line[0], init).facing;
    for (const dancerId of line) {
      if (getDancer(dancerId, init).facing.dot(refFacing) < 0.7) {
        throw new Error(
  ...
  return [
    {
      ...
      position: (id, frac, segInit) => {
        ...
        if (idx === 0) {
          ...
          const center = getDancer(line[1], segInit).pos;
          ...
          const center = getDancer(line[2], segInit).pos;
          return revolve(segInit[id].pos, {
            around: center,
            radians: (PI / 2) * frac,
          });
