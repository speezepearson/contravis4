...
export const doSiDoAnimator: InstructionAnimator<DoSiDoInstruction> = (init, who, instr) => {
  return (t) => produce(init, (draft) => {
    ...
    for (const id of who) {
      ...
      draft.protos[id].facing = draft.protos[id].facing.rotateByDegrees(360 * Math.floor(instr.beats/3) * progressFrac)
    }
  });
}
