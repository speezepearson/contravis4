...
import { otherHand, parseProtoId, resolveRelationship, type Beats, type Hand } from '../contraCore';
...
export const californiaTwirlAnimator: InstructionAnimator<CaliforniaTwirlInstruction> = (init, who, instr) => {
  const plans = buildProtoRecord((id) => {
    ...
    return {
      ...
      endFacing: getDir({from: myPos, to: theirPos}).rotateByDegrees(90 * (myRole === 'lark' ? 1 : -1)),
      twirlHand,
      relationship,
    }
  });
  return (t) => produce(init, (draft) => {
    ...
    for (const id of who) {
      const plan = plans[id];
      draft.protos[id].pos = ellipsePosition(plan.start, plan.end, 1/2, Math.PI*progressFrac);
      draft.protos[id].facing = lerpFacing(init.protos[id].facing, plan.endFacing, progressFrac);
      connectHands(draft, id, plan.twirlHand, plan.relationship, otherHand(plan.twirlHand));
    }
  });
};
